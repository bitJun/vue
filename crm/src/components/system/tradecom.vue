  ...
  <div class="setting_view cantainer">
    ...
    <div class="list">
      ...
      <table>
        <thead>
          <tr>
            <td v-bind:style="{width:'20%'}">规则名称</td>
            <td v-bind:style="{width:'20%'}">品种组</td>
            <td v-bind:style="{width:'15%'}">账户组</td>
            <td v-bind:style="{width:'15%'}">MT组</td>
            <td v-bind:style="{width:'10%'}">结算单位</td>
            <td v-bind:style="{width:'20%'}"><p>操作</p></td>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in json">
            <td v-bind:style="{width:'20%'}"><p>{{item.name}}</p></td>
            <td v-bind:style="{width:'20%'}"><p>{{item.type}}</p></td>
            <td v-bind:style="{width:'15%'}"><p>{{item.group}}</p></td>
            <td v-bind:style="{width:'15%'}"><p>{{item.MT}}</p></td>
            <td v-bind:style="{width:'10%'}"><p>{{item.unit}}</p></td>
            <td v-bind:style="{width:'20%'}">
              <p>
                <ul class="clearfix pull-right">
                  <li>
                    <a class="setting">
                      <i class="iconfont icon-shezhishedingpeizhibanshou"></i>
                    </a>
                  </li>
                  <li>
                    <a class="edit">
